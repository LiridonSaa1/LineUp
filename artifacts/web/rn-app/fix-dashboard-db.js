...
export const BarberDashboardScreen: React.FC<BarberDashboardScreenProps> = ({ user, onLogout }) => {
  ...
  useEffect(() => {
    async function loadDashboardData() {
      ...
      try {
        ...
        const appointmentsData = appts || [];
        setAppointments(appointmentsData);

        // Calculate Stats
        const confirmedAppts = appointmentsData.filter((a: any) => a.status === 'confirmed') || [];
        const revenue = confirmedAppts.reduce((sum: number, a: any) => sum + (parseInt(a.price) || 15), 0);

        setStats({
          todayRevenue: revenue,
          activeBookings: appointmentsData.length,
  ...
  const filteredAppointments = selectedStaffFilter 
    ? appointments.filter(a => a.employee_id === selectedStaffFilter)
    : appointments;
  ...
  return (
    <View className="flex-1 bg-[#F5F5F5]">
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
        ...
        {activeTab === 'staff' && (
          <View className="px-6">
            <View className="flex-row justify-between items-center mb-4">
              <Text className="text-lg font-black text-[#161719]">Punonjësit ({employees.length})</Text>
              <TouchableOpacity 
                onPress={async () => {
                  // Simulate adding staff if empty
                  if (employees.length === 0) {
                    ...
                    alert('Punonjësi u shtua me sukses! Rifreskoni faqen.');
                  } else {
                    alert('Modal për të shtuar staf (Së shpejti)');
                  }
                }}
                className="flex-row items-center bg-[#3473ef]/10 px-3 py-1.5 rounded-full"
              >
            ...
            {employees.length === 0 ? (
              <View className="bg-white p-8 rounded-3xl items-center justify-center border border-slate-100 border-dashed">
                <Users size={48} color="#CBD5E1" strokeWidth={1} className="mb-4" />
                <Text className="text-[#161719] font-black text-lg mb-1">Nuk keni Staf</Text>
                <Text className="text-[#8789A3] text-center font-bold text-sm">Klikoni "Shto" lart për të shtuar punonjësit tuaj.</Text>
              </View>
            ) : (
              employees.map(emp => {
                const empAppts = appointments.filter(a => a.employee_id === emp.id).length;
                return (
                  <View key={emp.id} className="bg-white p-4 rounded-2xl mb-4 border border-slate-100 shadow-sm flex-row items-center">
                    <Image source={{ uri: emp.image || 'https://via.placeholder.com/150' }} className="w-14 h-14 rounded-full mr-4 bg-slate-200" />
        ...
        {/* ===================== APPOINTMENTS TAB (TIMELINE DESIGN) ===================== */}
        {activeTab === 'appointments' && (
          <View className="px-6">
            ...
            <Text className="text-lg font-black text-[#161719] mb-4">Orari i Sotëm</Text>
            
            {filteredAppointments.length === 0 ? (
              <View className="bg-white p-8 rounded-3xl items-center justify-center border border-slate-100 border-dashed mt-4">
                ...
                <Text className="text-[#8789A3] text-center font-bold text-sm">
                  Nuk u gjet asnjë takim për këtë staf sot.
                </Text>
              </View>
            ) : (
              <View className="ml-2 relative pt-2">
                {/* Timeline vertical line */}
                <View className="absolute left-6 top-6 bottom-6 w-0.5 bg-slate-200" />
                
                {filteredAppointments.map((appt: any, idx: number) => {
                  const isConfirmed = appt.status === 'confirmed';
                  
                  return (
                    <View key={appt.id || idx} className="flex-row mb-6 relative z-10">
                      {/* Timeline dot & time */}
                      ...
                      <View className="flex-1 ml-4 bg-white rounded-3xl p-4 shadow-sm border border-slate-100">
                        <View className="flex-row justify-between items-start mb-3">
                          <View className="flex-1">
                            <Text className="font-black text-[#161719] text-base mb-1" numberOfLines={1}>
                              {appt.users?.name || 'Klient Anonim'}
                            </Text>
                            <Text className="text-[#8789A3] text-xs font-bold">{appt.service}</Text>
                          </View>
                          <Text className="font-black text-[#3473ef]">{appt.price}€</Text>
                        </View>
                        ...
                        <View className="flex-row justify-between items-center pt-3 border-t border-slate-50">
                          {appt.employees ? (
                            <View className="flex-row items-center">
                              <Image source={{ uri: appt.employees.image || 'https://via.placeholder.com/50' }} className="w-6 h-6 rounded-full mr-2 bg-slate-200" />
                              <Text className="text-[#8789A3] text-xs font-bold">Stafi: <Text className="text-[#161719]">{appt.employees.name}</Text></Text>
                            </View>
                          ) : (
                            <View className="flex-row items-center">
                              <View className="w-6 h-6 rounded-full bg-rose-50 items-center justify-center mr-2">
                                <AlertCircle size={12} color="#ef4444" />
                              </View>
                              <Text className="text-rose-500 text-xs font-bold">I pacaktuar</Text>
                            </View>
                          )}
