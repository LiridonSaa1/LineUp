...
import { Mail, Phone, MapPin, Clock, Send, CheckCircle2, Scissors } from "lucide-react";
  ...
  const info = [
    {
      icon: Mail,
      label: "Email",
      value: "info@trimkosova.com",
      sub: "Përgjigje brenda 24 orëve",
    },
    {
      icon: Phone,
      label: "Telefon",
      value: "+383 44 123 456",
      sub: "E hënë – E premte, 09:00–18:00",
    },
    {
      icon: MapPin,
      label: "Adresa",
      value: "Rr. Nëna Terezë, Prishtinë",
      sub: "Kosovë, 10000",
    },
    {
      icon: Clock,
            ...
            <div className="p-5 rounded-2xl bg-primary/5 border border-primary/15">
              ...
              <div className="flex gap-2">
                {["Instagram", "Facebook", "TikTok"].map(s => (
                  <button key={s}
                    className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-border/60 hover:border-primary/40 hover:text-primary transition-colors"
                  >
                    {s}
                  </button>
                ))}
              ...
              {sent ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <button
                    type="submit"
                    disabled={sending}
                    className="w-full flex items-center justify-center gap-2.5 py-3.5 rounded-xl bg-primary text-white font-bold text-base shadow-lg shadow-primary/25 hover:shadow-primary/40 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                  >
                    {sending ? (
                      <>
