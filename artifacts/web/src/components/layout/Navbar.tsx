...
const DARK_PILL = "bg-zinc-900/85 backdrop-blur-xl shadow-lg shadow-black/30 border border-white/8";
...
export function Navbar() {
  ...
  // On non-home pages always show the pill layout
  ...
  // ── Section lists ──────────────────────────────────────
  // All home sections in order (for transparent state)
  const allSections = [
    { id: "home",          label: "Home" },
  ...
  // ── Auth section (shared across both layouts) ──────────
  const AuthSection = () => (
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-40">
        ...
        {/* ══════════════════════════════════════════════════════
            STATE A — Transparent hero navbar (home, not scrolled)
            Logo small left | all links center | auth right
        ══════════════════════════════════════════════════════ */}
        <div
          className={`hidden md:block transition-all duration-400 ${
            showPill ? "opacity-0 pointer-events-none h-0 overflow-hidden" : "opacity-100"
          }`}
        >
          <div className="flex items-center justify-between px-8 py-4">
            {/* Small logo left */}
            <button onClick={() => scrollToSection("home")} className="flex items-center group shrink-0" aria-label="Home">
              <img
                src={logoImg}
                alt="Line UP"
        ...
        {/* ══════════════════════════════════════════════════════
            STATE B — Dark pill (scrolled or non-home)
            3-col: left nav | big logo center | right nav + auth
        ══════════════════════════════════════════════════════ */}
        <div
          className={`hidden md:block transition-all duration-400 ${
            !showPill ? "opacity-0 pointer-events-none h-0 overflow-hidden" : "opacity-100 py-2"
          }`}
        >
          <div className="mx-auto max-w-7xl px-4">
            <div className={`grid grid-cols-[1fr_auto_1fr] items-center rounded-2xl px-5 py-1 ${DARK_PILL}`}>
        ...
        {/* ══════════════════════════════════════════════════════
            MOBILE — always dark pill
        ══════════════════════════════════════════════════════ */}
        <div className="md:hidden py-2 px-4">
          <div className={`flex items-center justify-between rounded-2xl px-4 py-2.5 ${DARK_PILL}`}>
              ...
              ? allSections.map((s) => (
                  <button
                    ...
                    className={`px-4 py-3 rounded-xl text-sm font-medium transition-all text-left ${
                      sectionIsActive(s.id) ? "text-foreground bg-primary/8 font-semibold" : "text-muted-foreground hover:text-foreground hover:bg-black/5"
                    }`}
                  >
                    {s.label}
                  </button>
      ...
      {/* Spacer — only on non-home pages (pill height = 124px) */}
